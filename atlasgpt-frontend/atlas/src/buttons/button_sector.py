...
def firstinstruction():
    instruction = (
            ...
            You are very important and decisive in helping us find our user's business sector.
...
def sector(user_input, user_token, history, payload, modelurl):
    ...
    mpisector(user_token, history, history_file_path)

def mpisector(user_token, history, history_file_path):
    ...
    if user_data["economic_sector"] != None:
        print("ESTAMOS QUASE")
        ...
        with open(f"atlas/database/mpi_Rio_Grande_do_Sul.csv", "r",  newline='', encoding='utf-8') as file:
            ...
            matching_rows = [row for row in mpi_city if row["CNAE_section"] == user_data["economic_sector"]]
            matching_rows.sort(key=lambda row: float(row["MPI"]), reverse=True)

        best_imp = ''
        for i, row in enumerate(matching_rows):
            if i == 1:
                best_imp += f"Melhor cidade para abrir o negócio: {row['NM_MUN']}"+"\n"
            if i == 2:
                best_imp += f"Segunda melhor cidade para abrir o negócio: {row['NM_MUN']}"+"\n"
            if i == 3:
                best_imp += f"Terceira melhor cidade para abrir o negócio: {row['NM_MUN']}"+"\n"
            if i == 4:
                best_imp += f"Quarta melhor cidade para abrir o negócio: {row['NM_MUN']}"+"\n"
            if i == 5:
                best_imp += f"Quinta melhor cidade para abrir o negócio: {row['NM_MUN']}"
        # print(user_data["economic_sector"], matching_rows)
        print(best_imp)
        instruction = (
                f'''
                Follow these instructions and inform the user immediately.
                The best place to open the business has been found!
                Based on the Market Potential Index (IPM) calculation, we have the result.
                The calculation was made for the chosen sector ({user_data["economic_sector"]}).
                The best places to open the business are:
                {best_imp}
                Explain this to the user.
                Send a message about it.
                Speak in a very natural way.
                Write in the language choosed by the user
                '''
            )
