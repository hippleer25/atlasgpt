...
def get_profile(user_token):
    ...
    if len(business_data) > 0:
        business_info = business_data[-1]
        user_data["region"] = business_info.get('region')
        user_data["state"] = business_info.get('state')
        user_data["city"] = business_info.get('city')
        user_data["neighborhood"] = business_info.get('neighborhood')
        user_data["business_info"] = business_info
        user_data["rent"] = business_info.get('rent')
        user_data["public_politics"] = business_info.get('public_politics')
        user_data["economic_sector"] = business_info.get('economic_sector')
    else:
        user_data["region"], user_data["state"], user_data["city"], user_data["neighborhood"], user_data["rent"], user_data["public_politics"], user_data["economic_sector"] = None, None, None, None, None, None, None
