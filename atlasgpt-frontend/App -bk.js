...
const App = () => {
  ...
  const translations = {
    en: {
      ...
      button4: 'I do not know the location or the sector yet',
      termsText:
    ...
    pt: {
      ...
      button2: 'Eu já tenho um setor',
      button3: 'Eu tenho um setor e uma localização',
      button4: 'Ainda não sei o local nem o setor',
      termsText:
  ...
  const sendMessage = async (text) => {
    ...
    try {
      const response = await fetch(
        `http://143.54.117.114:5000/api?q=${encodeURIComponent(
          trimmedText
        ...
        {isButtonsVisible && (
          <View style={styles.buttonContainer}>
            <View style={styles.infoButtonsGrid}>
              ...
              <TouchableOpacity
                style={styles.infoButton}
                onPress={() => handleButtonPress(currentTranslations.button4)}
              >
                <Text style={styles.infoButtonText}>{currentTranslations.button4}</Text>
              </TouchableOpacity>
            </View>
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  infoButton: {
    ...
    borderColor: '#e2e8f0',
    flex: 1,
    minWidth: Platform.OS === 'web' ? 300 : '45%',
    shadowColor: '#000',
    shadowOffset: { width: 0, height: 1 },
    shadowOpacity: 0.05,
    shadowRadius: 2,
